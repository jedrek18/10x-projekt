  ...
  function from(table: string) {
    ...
    const selectBuilder = {
      ...
      single: async () => {
        ...
        return {
          data: options.settingsRow ?? { user_id: options.user?.id, daily_goal: 20, new_limit: 10 },
          error: null,
        } as any;
      },
    ...
    const insertBuilder = {
      ...
      ignore() {
        return { error: options.insertErrorCode ? { code: options.insertErrorCode } : null } as any;
      },
    } as any;
    ...
    const updateBuilder = {
      ...
      then: options.makeUpdatePending
        ? makeThenable(new Promise(() => {})).then
        : makeThenable({
            error: options.updateErrorCode ? { code: options.updateErrorCode, message: "constraint" } : null,
          }).then,
    } as any;
...
describe("/api/user-settings endpoints", () => {
  it("GET 200 returns settings for authenticated user (auto-create)", async () => {
    const supabase = createMockSupabase({
      user: { id: "u1" },
      settingsRow: { user_id: "u1", daily_goal: 20, new_limit: 10 },
  ...
  it("PATCH 200 updates settings for authenticated user", async () => {
    const supabase = createMockSupabase({
      user: { id: "u1" },
      settingsRow: { user_id: "u1", daily_goal: 30, new_limit: 5 },
      insertErrorCode: null,
    });
    const req = new Request("http://localhost/api/user-settings", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ daily_goal: 30 }),
    });
  ...
  it("PATCH 422 on validation errors", async () => {
    ...
    const req = new Request("http://localhost/api/user-settings", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ daily_goal: 0 }),
    });
    const res = await UserSettingsPATCH({ request: req, locals: { supabase } } as any);
  ...
  it("PATCH 422 on DB constraint violation", async () => {
    ...
    const req = new Request("http://localhost/api/user-settings", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ daily_goal: 9999 }),
    });
    const res = await UserSettingsPATCH({ request: req, locals: { supabase } } as any);
    expect(res.status).toBe(422);
  });
