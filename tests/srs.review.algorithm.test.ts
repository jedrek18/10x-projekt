...
function createMockSupabaseReview(options: {
  userId: string;
  card: any;
  progress?: { reviews_done?: number } | null;
}) {
  const state: any = {
    card: options.card,
  ...
  function from(table: string) {
    const chain: any = {
      ...
      eq() { return chain; },
      is() { return chain; },
      update(vals?: any) { chain._updateVals = vals; return chain; },
      async maybeSingle() {
      ...
      then(onFulfilled: any) { return Promise.resolve({ data: [], error: null }).then(onFulfilled); },
    };
    return chain as any;
  }
...
describe("reviewCard algorithm", () => {
  ...
  it("rating=1 moves to learning with ~1 day due", async () => {
    const now = Date.now();
    const supabase = createMockSupabaseReview({ userId: "u1", card: { id: "c1", state: "new", interval_days: 0, ease_factor: 2.5, reps: 0, lapses: 0 } });
  ...
  it("rating=2 sets review and increases interval with min 1", async () => {
    const supabase = createMockSupabaseReview({ userId: "u1", card: { id: "c1", state: "learning", interval_days: 0, ease_factor: 2.5, reps: 1, lapses: 0 } });
  ...
  it("rating=3 sets review and grows interval and ease", async () => {
    const supabase = createMockSupabaseReview({ userId: "u1", card: { id: "c1", state: "review", interval_days: 2, ease_factor: 2.3, reps: 3, lapses: 0 } });
