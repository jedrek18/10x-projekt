...
function createMockSupabasePromote(options: {
  ...
  const auth = { async getUser() { return { data: { user: { id: options.userId } }, error: null } as any; } } as const;
...
describe("promote-new limits", () => {
  it("returns empty when no allowance left", async () => {
    ...
    const req = new Request(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ count: 5 }) });
  ...
  it("caps by soft per-day cap and remaining allowance", async () => {
    const supabase = createMockSupabasePromote({ userId: "u1", newLimit: 10, introduced: 2, candidates: ["c1", "c2", "c3", "c4", "c5", "c6"] });
    const url = new URL("http://localhost/api/srs/promote-new");
    const req = new Request(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ count: 100 }) });
    const res = await SrsPromotePOST({ request: req, locals: { supabase } } as any);
    const body = await readJson(res);
