...
import { queueQuerySchema } from "../../../lib/validation/srs";
import { buildQueue } from "../../../lib/services/srs.service";
...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    const parsed = queueQuerySchema.safeParse(queryObject);
    if (!parsed.success) {
    ...
    const result = (await Promise.race([
      buildQueue(supabase, parsed.data.goal_hint),
      new Promise((_, reject) => {
        const err = new Error("Request timeout");
        (err as any).name = "AbortError";
        setTimeout(() => reject(err), TIMEOUT_MS);
      }),
    ])) as Awaited<ReturnType<typeof buildQueue>>;

    const headers = new Headers({ "Cache-Control": "private, max-age=0" });
    return json(result, { status: 200, headers });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error("[api/srs/queue] GET failed", error);
    try { await logError(locals?.supabase, { endpoint: "/api/srs/queue", error }); } catch {}
